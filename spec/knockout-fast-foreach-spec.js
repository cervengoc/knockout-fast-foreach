...
[0, 2].forEach(function (partitionSize) {
  
  describe("applying bindings",
    function() {
      setupSynchronousFrameAnimation()

      before(function () {
        FastForEach.defaultPartitionSize = partitionSize;
      });
      
      it("works with a static list",
        function() {
      ...
      it("works with an observable array",
        function() {
          var target = $("<ul data-bind='fastForEach: $data'><li data-bind='text: $data'></li></div>");
          var list = [1, 2, 3];
      ...
      it("works with a plain observable with an array",
        function() {
          var target = $("<ul data-bind='fastForEach: $data'><li data-bind='text: $data'></li></div>");
          var list = [1, 2, 3];
      ...
      it("works with a computed observable",
        function() {
          var target = $("<ul data-bind='fastForEach: $data'><li data-bind='text: $data'></li></div>");
          var list = [1, 2, 3];
      ...
      it("applies bindings to the immediate child",
        function() {
          var target = $("<ul data-bind='fastForEach: $data'><li data-bind='text: $data'></li></div>");
          var list = ['a', 'b', 'c'];
          ko.applyBindings(list, target[0])
      ...
      it("bindings only inner (virtual) element",
        function() {
      ...
      it("bindings mixed inner virtual elements",
        function() {
          var target = $("<ul data-bind='fastForEach: $data'>Q<!-- ko text: $data -->Z2<!-- /ko -->R</ul>");
          var list = ['E2', 'V2'];
          ko.applyBindings(list, target[0])
          assert.equal(target.html(),
            'Q<!-- ko text: $data -->E2<!-- /ko -->R' +
            'Q<!-- ko text: $data -->V2<!-- /ko -->R')
        })

      it("uses the name/id of a <template>",
        function() {
          ...
          ko.applyBindings(list, target[0])
          assert.equal(target.html(),
            "X<!-- ko text: $data-->F1<!--/ko-->" +
            "X<!-- ko text: $data-->F2<!--/ko-->");
          $template.remove();
        })

      it("uses the name/id of a <script>",
        function() {
          var target = $("<ul data-bind='fastForEach: {name: \"tID\", data: $data}'>Zee</ul>");
          var list = ['G1', 'G2'];
          ...
          ko.applyBindings(list, target[0])
          assert.equal(target.html(),
            "Y<!-- ko text: $data-->G1<!--/ko-->" +
            "Y<!-- ko text: $data-->G2<!--/ko-->");
          $template.remove();
        })

      it("uses the name/id of a <div>",
        function() {
          ...
          ko.applyBindings(list, target[0])
          assert.equal(target.html(),
            "Z<!-- ko text: $data-->H1<!--/ko-->" +
            "Z<!-- ko text: $data-->H2<!--/ko-->");
          $template.remove();
        })
    })

  describe("observable array changes",
    function () {
      ...
      it("adds an item to the end of a pre-existing list",
        function() {
          obs(['a'])
          ko.applyBindings(view, div[0]);
          obs.push('b')
      ...
      it("adds an item to the beginning of a pre-existing list",
        function() {
          obs(['a'])
          ko.applyBindings(view, div[0]);
          obs.unshift('b')
      ...
      it("adds an item to the middle of a pre-existing list",
        function() {
          obs(['a', 'b'])
          ko.applyBindings(view, div[0]);
          obs.splice(1, 0, 'c')
      ...
      it("splices items at the beginning of a pre-existing list",
        function() {
          obs(['a', 'b', 'c'])
          ko.applyBindings(view, div[0]);
          obs.splice(0, 1, 'd')
      ...
      it("removes items at the middle of a pre-existing list",
        function() {
          obs(['a', 'b', 'c'])
          ko.applyBindings(view, div[0]);
          obs.splice(0, 1)
      ...
      it("splices items at the middle of a pre-existing list",
        function() {
          obs(['a', 'b', 'c'])
          ko.applyBindings(view, div[0]);
          obs.splice(1, 1, 'D')
      ...
      it("splices items at the end of a pre-existing list",
        function() {
          obs(['a', 'b', 'c'])
          ko.applyBindings(view, div[0]);
          obs.splice(2, 1, 'D')
      ...
      it("deletes from virtual elements",
        function() {
          div = $("<div>")
      ...
      it("combines multiple adds and deletes",
        function() {
          obs(['A', 'B', 'C', 'D', 'E', 'F'])
          ko.applyBindings(view, div[0]);
          obs(['x', 'B', 'C', 'D', 'z', 'F'])
      ...
      it("processes numerous changes with splice",
        function() {
          ko.applyBindings(view, div[0]);
          obs([5, 6, 7, 8, 9])
          assert.equal(div.text(), '56789')
      ...
      it("accepts changes via a computed observable",
        function() {
          var target = $("<ul data-bind='fastForEach: $data'><li data-bind='text: $data'></li></div>");
      ...
      describe("DOM move capabilities",
        function() {
          it("sorting complex data moves 1 DOM node",
            function() {
          ...
          it("sorting complex data moves all DOM nodes",
            function() {
              div = $("<div data-bind='fastForEach: obs'><div data-bind='html: testHtml'></div></div>");
              ko.applyBindings(view, div[0]);
          ...
          it("sorting complex data recreates DOM nodes if move disabled",
            function() {
          ...
          it("Sort large complex array makes correct DOM moves",
            function() {
              var itemNumber = 100;
          ...
          it("Sort large complex array makes correct DOM order without move",
            function() {
              var originalShouldDelayDeletion = FastForEach.prototype.shouldDelayDeletion;
              FastForEach.prototype.shouldDelayDeletion = function(data) { return false; }
              var itemNumber = 100;
          ...
          it("processes changes from more changesets 1",
            function() {
              var originalAnimateFrame = FastForEach.animateFrame;
              FastForEach.animateFrame = null;
          ...
          it("processes changes from more changesets 2",
            function() {
              var originalAnimateFrame = FastForEach.animateFrame;
              FastForEach.animateFrame = null;
              div = $("<div data-bind='visible: true'></div>");
      ...
      describe('partitioned processing',
        function() {

          it("partitioned processing",
            function() {
              var calls = 0;
              var nodes = 0;
              ...
              function cb(v) {
                calls++;
                nodes += v.nodesInserted.length
              }

              var target = $("<ul data-bind='fastForEach: { data: arr, afterAdd: cb, partition: 100 }'><li data-bind='text: $data'></li></div>");
              ko.applyBindings({ arr: arr, cb: cb }, target[0])
              assert.equal(calls, 10)
              assert.equal(nodes, 1000)
            });
      ...
      describe('afterAdd',
        function() {
          it("emits on changes to an observable array",
            function() {
              var calls = 0;
              var nodes = 0
              var arr = ko.observableArray([])
              ...
              arr([2, 3, 4])
              assert.equal(calls, 1 + (partitionSize ? Math.ceil(arr().length / partitionSize) : 1))
              assert.equal(nodes, 4, 'n4')
            })

          it("is called with initial data",
            function() {
              var calls = 0;
              var nodes = 0
              ...
              var target = $("<ul data-bind='fastForEach: { data: arr, afterAdd: cb }'><li data-bind='text: $data'></li></div>");
              ko.applyBindings({ arr: arr, cb: cb }, target[0])
              assert.equal(calls, (partitionSize ? Math.ceil(arr().length / partitionSize) : 1))
      ...
      describe('beforeRemove',
        function() {
          ...
          it("removes an element if a `then`-able is passed",
            function() {
              var cbi = 0;
      ...
      describe("$index",
        function() {
          ...
          it("is present on children of virtual nodes",
            function() {
          ...
          it("is present when template starts with a text node",
            function() {
          ...
          it("updates the first list item",
            function() {
              var target = $("<ul data-bind='fastForEach: $data'><li data-bind='text: $data'></li></div>");
